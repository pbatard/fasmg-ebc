...
EFI_STATUS efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable)
{
	...
	// Test max number of parameters
	Status = CustomProtocol->MaxParams64(
		0x0B0B0B0B0A0A0A0AULL, 0x1B1B1B1B1A1A1A1AULL,
		0x2B2B2B2B2A2A2A2AULL, 0x3B3B3B3B3A3A3A3AULL,
		0x4B4B4B4B4A4A4A4AULL, 0x5B5B5B5B5A5A5A5AULL,
		0x6B6B6B6B6A6A6A6AULL, 0x7B7B7B7B7A7A7A7AULL,
		0x8B8B8B8B8A8A8A8AULL, 0x9B9B9B9B9A9A9A9AULL,
		0xABABABABAAAAAAAAULL, 0xBBBBBBBBBABABABAULL,
		0xCBCBCBCBCACACACAULL, 0xDBDBDBDBDADADADAULL,
		0xEBEBEBEBEAEAEAEAULL, 0xFBFBFBFBFAFAFAFAULL);
	Print(L"MaxParams64: %s", EFI_ERROR(Status) ? L"FAIL" : L"PASS\n");
	if EFI_ERROR(Status) {
		Print(L" - [%x] %r\n", Status & 0x7FFFFFFF, Status);
		return Status;
	}
	Status = CustomProtocol->MaxParamsMixed(
		0x0A0A0A0A, 0x1B1B1B1B1A1A1A1AULL,
		0x2A2A2A2A, 0x3B3B3B3B3A3A3A3AULL,
		0x4A4A4A4A, 0x5B5B5B5B5A5A5A5AULL,
		0x6A6A6A6A, 0x7B7B7B7B7A7A7A7AULL,
		0x8A8A8A8A, 0x9B9B9B9B9A9A9A9AULL,
		0xAAAAAAAA, 0xBBBBBBBBBABABABAULL,
		0xCACACACA, 0xDBDBDBDBDADADADAULL,
		0xEAEAEAEA, 0xFBFBFBFBFAFAFAFAULL);
	Print(L"MaxParamsMixed: %s", EFI_ERROR(Status) ? L"FAIL" : L"PASS\n");
	if EFI_ERROR(Status) {
		Print(L" - [%x] %r\n", Status & 0x7FFFFFFF, Status);
		return Status;
	}
